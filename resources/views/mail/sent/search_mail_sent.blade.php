...
<p onclick="window.location = '{{ route('mail_sent_show',$mail )}}'" class="w-1/2 lg:w-1/5 cursor-pointer py-3 text-center text-sm text-gray-200 group-hover:text-teal-600">
  {{ $mail->recipient ?: '/' }}
</p>
<p onclick="window.location = '{{ route('mail_sent_show',$mail )}}'" class="w-1/5 hidden lg:block cursor-pointer py-3 text-center text-sm text-gray-200 group-hover:text-teal-600">
  {{ $mail->subject ?: '/' }}
</p>
<p onclick="window.location = '{{ route('mail_sent_show',$mail )}}'" class="w-1/2 lg:w-1/5 cursor-pointer py-3 text-center text-sm text-gray-200 group-hover:text-teal-600">
  {{ $mail->created_at->format('d-m-Y H:m') }}
</p>
<p onclick="window.location = '{{ route('mail_sent_show',$mail )}}'" class="w-1/5 hidden lg:block cursor-pointer py-3 text-center text-sm text-teal-400 group-hover:text-teal-600">
