...
<p onclick="window.location = '{{ route('mail_inbox_show_deleted',$mailDeleted )}}'" class="w-1/4 lg:w-1/6 cursor-pointer py-3 text-center text-sm text-gray-200 group-hover:text-teal-600">
  {{ $mailDeleted->name ?: '/' }}
</p>
<p onclick="window.location = '{{ route('mail_inbox_show_deleted',$mailDeleted )}}'" class="w-1/4 lg:w-1/6 cursor-pointer py-3 text-left md:text-center text-sm text-gray-200 group-hover:text-teal-600">
  {{ $mailDeleted->from ?: '/' }}
</p>
<p onclick="window.location = '{{ route('mail_inbox_show_deleted',$mailDeleted )}}'" class="w-1/6 hidden lg:block cursor-pointer py-3 text-left md:text-center text-sm text-gray-200 group-hover:text-teal-600">
  {{ $mailDeleted->subject ?: '/' }}
</p>
<p onclick="window.location = '{{ route('mail_inbox_show_deleted',$mailDeleted )}}'" class="w-1/4 lg:w-1/6 cursor-pointer py-3 text-left md:text-center text-sm text-gray-200 group-hover:text-teal-600">
  {{ $mailDeleted->created_at->format('d-m-Y H:m') }}
</p>
<p onclick="window.location = '{{ route('mail_inbox_show_deleted',$mailDeleted )}}'" class="w-1/6 hidden lg:block cursor-pointer py-3 text-left md:text-center text-sm text-teal-400 group-hover:text-teal-600">
  <i class="transition ease-out duration-500 transform hover:scale-110 far fa-eye fa-lg"></i>
</p>
<p onclick="window.location = '{{ route('mail_inbox_restore_deleted',$mailDeleted )}}'" class="w-1/6 hidden lg:block cursor-pointer py-3 text-left md:text-center text-sm text-teal-400 group-hover:text-teal-600">
...
<p class="w-1/6 hidden lg:block cursor-pointer get_route_id text-red-600 hover:text-red-700 text-left md:text-center">
  ...
  <span class="hidden">{{route('mail_permanently_delete', $mailDeleted)}}</span>
   <em class="hidden">{{ $mailDeleted->from }}</em>
</p>
</div>
@endforeach
<div class="px-4 py-2">
...
@foreach($mailInboxDeleted as $mailDeleted)
      <div onclick="window.location = '{{ route('mail_inbox_show_deleted',$mailDeleted )}}'" class="cursor-pointer border-b border-teal-400 rounded-xl px-2 py-1 my-5 text-sm shadow-lg bg-gray-800">
