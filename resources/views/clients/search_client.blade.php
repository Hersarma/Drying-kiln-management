...
<p onclick="window.location = '{{ route('clients.show',$client )}}'" class="sm:w-1/2 md:w-1/2 lg:w-1/5 cursor-pointer py-3 text-center text-sm text-gray-200 group-hover:text-teal-600">
  {{ ucfirst($client->name) }}
</p>
<p onclick="window.location = '{{ route('clients.show',$client )}}'" class="sm:w-1/2 md:w-1/2 lg:w-1/5 hidden sm:block md:block lg:block cursor-pointer py-3 text-center text-sm text-gray-200 group-hover:text-teal-600">
  {{ ucfirst($client->email) }}
</p>
<p onclick="window.location = '{{ route('clients.show',$client )}}'" class="w-1/5 hidden lg:block cursor-pointer py-3 text-center text-sm text-gray-200 group-hover:text-teal-600">
  {{ $client->notes }}
</p>
<p onclick="window.location = '{{ route('clients.show',$client )}}'" class="w-1/5 hidden lg:block cursor-pointer py-3 text-center text-sm text-teal-400 group-hover:text-teal-600">
...
@foreach($clients as $client)
      <div onclick="window.location = '{{ route('clients.show',$client )}}'" class="cursor-pointer border-b border-teal-400 rounded-xl px-2 py-1 my-5 text-sm shadow-lg bg-gray-800">
