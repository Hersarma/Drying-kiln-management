  ...
  <div class="md:px-8 md:flex justify-between w-full text-center bg-gradient-to-r from-blue_gray-900 via-blue_gray-800 to-blue_gray-900">
    <div class="px-8 py-8">
      ...
      <div class="flex items-center text-gray-600 px-4 md:px-12 py-4 md:py-8">
        ...
        <input type="search" name="search_users" placeholder="Pretraga"
        class="search_users bg-transparent text-gray-100 border-b border-gray-200 focus:outline-none">
    ...
    <form method="post" action="{{ route('delete_checked_users') }}">
      ...
      <div>
        ...
        <div class="searchUsers">
          @include('settings.users.search_user')
        </div>
      </div>
      @if($users->isNotEmpty())
...
<section class="sm:hidden md:hidden lg:hidden">
    <div class="w-full my-3 px-4">
        <p class="text-lg text-gray-200 text-center py-3">Korisnici</p>
        <div class="toggle_modal_create_user flex justify-between items-center mx-auto text-white bg-teal-400 border-0 py-2 px-4 focus:outline-none hover:bg-teal-500 rounded text-base shadow-lg shadow-teal-400/20 mb-3">
          <button type="button" class="">Novi korisnik</button>
          <i
          class="fa fa-plus"></i>
        </div>
        
        <input type="search" name="search_users" id="search_users" placeholder="Pretraga"
        class="search_users appearance-none bg-gradient-to-r from-blue_gray-800 via-blue_gray-700 to-blue_gray-800 rounded-xl border-l-4 border-gray-400 w-full py-3 px-4 text-gray-200 leading-tight focus:outline-none focus:shadow-xl focus:shadow-teal-400/20">
    </div>
    <div class="searchUsers">
     @include('settings.users.search_user')
    </div>
