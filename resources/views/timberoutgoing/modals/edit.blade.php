<div class="modal_edit_timber_outgoing hidden fixed z-20 inset-0 overflow-y-auto">
	<div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
		...
		<div class="inline-block rounded-xl border-l-4 border-turquoise-light bg-gradient-to-r from-blue_gray-900 via-blue_gray-800 to-blue_gray-900 overflow-hidden transform transition-all sm:my-8 align-middle w-full md:w-2/3 shadow-2xl">
			...
			<div class="w-full max-w-5xl mx-auto">
				<form method="post" action="{{ route('timberoutgoing.update', $timberoutgoing) }}" class="py-8 px-8 md:px-0">
					@csrf
					@method('PATCH')
					...
					<div class="md:flex md:items-center mb-6 text-gray-200 text-opacity-80 focus-within:text-opacity-100">
						...
						<div class="relative md:w-2/3">
							...
							<p class="text-red-500 text-sm italic mt-4">
								{{ $errors->edit_timber_outgoing->first('client_id') }}
							</p>
							<div class="clients h-96 overflow-auto hidden absolute z-50 mt-4 w-full bg-blue_gray-800 rounded-xl border-l-4 border-turquoise-light w-full py-3 px-8 text-gray-200 leading-tight">
								...
								<ul id="searchClient">
									@include('timberoutgoing.search_client')
								</ul>
							</div>
						</div>
					</div>
					<div class="md:flex md:items-center mb-6 text-gray-200 text-opacity-80 focus-within:text-opacity-100">
		                ...
		                <div class="md:w-2/3">
		                  ...
		                  id="transport_company" name="transport_company" value="{{ $timberoutgoing->transport_company }}">
		                  <p class="text-red-500 text-sm italic mt-4">
		                    {{ $errors->edit_timber_outgoing->first('transport_company') }}
		                  </p>
		                </div>
		            </div>
		            <div class="md:flex md:items-center mb-6 text-gray-200 text-opacity-80 focus-within:text-opacity-100">
		                ...
		                <div class="md:w-2/3">
		                  <input class="appearance-none bg-gradient-to-r from-blue_gray-800 via-blue_gray-700 to-blue_gray-800 rounded-xl border-l-4 border-gray-400 w-full py-3 px-4 text-gray-200 leading-tight focus:outline-none focus:border-turquoise-light focus:shadow-xl shadow-lg"
		                  id="invoice_number" name="invoice_number" value="{{ $timberoutgoing->invoice_number }}">
		                  <p class="text-red-500 text-sm italic mt-4">
		                    {{ $errors->edit_timber_outgoing->first('invoice_number') }}
		                  </p>
		                </div>
		            </div>
					<div class="md:flex md:items-center mb-6 text-gray-200 text-opacity-80 focus-within:text-opacity-100">
					...
					<div class="flex justify-between py-4">
						<button type="button"
						class="close_modal_edit_timber_outgoing py-2 w-1/3 transition ease-out duration-500 transform hover:scale-110 py-2 px-4 border border-transparent text-sm leading-5 font-medium rounded-md text-black bg-gray-300 hover:bg-gray-400 focus:outline-none">
						Otkaži
						</button>
						<button type="submit"
					...
					<div>
						@if($errors->edit_timber_outgoing->any())
