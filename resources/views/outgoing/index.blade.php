    ...
    <form method="post" action="{{ route('delete_checked_outgoing') }}">
      ...
      <div>
        <div class="flex justify-between items-center px-4 border-b border-turquoise-light">
          ...
          <p class="w-1/5 py-3 tracking-wider text-gray-100 text-sm text-center bg-gradient-to-r from-blue_gray-900 via-blue_gray-800 to-blue_gray-900">
            Klijent
          </p>
          <p class="w-1/5 py-3 tracking-wider text-gray-100 text-sm text-center bg-gradient-to-r from-blue_gray-900 via-blue_gray-800 to-blue_gray-900">
            Beleške
...
<section class="sm:hidden md:hidden lg:hidden">
    <div class="w-full my-3 px-4">
        <p class="text-lg text-gray-200 text-center py-3">Prikaz izlaza</p>
        <div class="toggle_modal_create_outgoing flex justify-between items-center mx-auto text-white bg-teal-400 border-0 py-2 px-4 focus:outline-none hover:bg-teal-500 rounded text-base shadow-lg shadow-teal-400/20 mb-3">
          <button type="button" class="">Novi izlaz</button>
          <i
          class="fa fa-plus"></i>
        </div>
        <input type="search" name="search_outgoing" id="search_outgoing" placeholder="Pretraga"
        class="search_outgoing appearance-none bg-gradient-to-r from-blue_gray-800 via-blue_gray-700 to-blue_gray-800 rounded-xl border-l-4 border-gray-400 w-full py-3 px-4 text-gray-200 leading-tight focus:outline-none focus:border-turquoise-light focus:shadow-xl focus:shadow-teal-400/20">
    </div>
    <div class="searchOutgoing">
     @include('outgoing.search')
    </div>
