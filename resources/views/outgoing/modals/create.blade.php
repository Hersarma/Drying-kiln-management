<div class="modal_create_outgoing hidden fixed z-20 inset-0 overflow-y-auto">
	<div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
		...
		<div class="inline-block rounded-xl border-l-4 border-turquoise-light bg-gradient-to-r from-blue_gray-900 via-blue_gray-800 to-blue_gray-900 overflow-hidden transform transition-all sm:my-8 align-middle w-full md:w-2/3 shadow-2xl">
			...
			<div class="w-full max-w-5xl mx-auto">
				<form method="post" action="{{ route('outgoing.store') }}" class="py-8 px-8 md:px-0">
					@csrf
					...
					<div class="md:flex md:items-center mb-6 text-gray-200 text-opacity-80 focus-within:text-opacity-100">
		                ...
		                <div class="md:w-2/3">
		                  ...
		                  <p class="text-red-500 text-sm italic mt-4">
		                    {{ $errors->create_outgoing->first('transport_company') }}
		                  </p>
		                </div>
		            </div>
		            <div class="md:flex md:items-center mb-6 text-gray-200 text-opacity-80 focus-within:text-opacity-100">
		                ...
		                <div class="md:w-2/3">
		                  ...
		                  <p class="text-red-500 text-sm italic mt-4">
		                    {{ $errors->create_outgoing->first('invoice_number') }}
		                  </p>
		                </div>
		            </div>
					<div class="md:flex md:items-center mb-6 text-gray-200 text-opacity-80 focus-within:text-opacity-100">
						...
						<div class="md:w-2/3">
							...
							<p class="text-red-500 text-sm italic mt-4">
								{{ $errors->create_outgoing->first('notes') }}
							</p>
						</div>
					</div>
					...
					<div class="flex justify-between py-4">
						<button type="button"
						class="toggle_modal_create_outgoing py-2 w-1/3 transition ease-out duration-500 transform hover:scale-110 py-2 px-4 border border-transparent text-sm leading-5 font-medium rounded-md text-black bg-gray-300 hover:bg-gray-400 focus:outline-none">
						Otkaži
						</button>
						<button type="submit"
...
<script type="text/javascript">
    ...
    $("#add").click(function(){
        ++i;
        $("#items").append('<tr class="bg-gradient-to-r from-blue_gray-900 via-blue_gray-800 to-blue_gray-900 border-b border-gray-700"><td class="px-4 py-3 text-left md:text-center text-gray-700"><input class="py-1 text-xl" type="text" name="items['+i+'][item_name]"></td><td class="px-4 py-3 text-left md:text-center text-gray-700"><input class="py-1 text-xl" type="text" name="items['+i+'][quantity]"></td><td class="px-4 py-3 text-left md:text-center text-gray-700"><input class="comma py-1 text-xl" type="text" name="items['+i+'][cubic_metre]"></td><td class="px-4 py-3 text-left md:text-center text-gray-700"><button type="button" class="remove-tr transition ease-out duration-500 transform hover:scale-110 py-2 px-4 border border-transparent text-sm leading-5 font-medium rounded-md text-white bg-red-400 hover:bg-red-500 focus:outline-none shadow-xl"><i class="fa fa-minus" aria-hidden="true"></i></button></td></tr>');
